...
std::shared_ptr<IRequestHandler> BaseRequestHandlerFactory::CreateInstance(short type)
{
	if (m_creators.count(type) == 0)
		throw std::exception{ "해당 요청을 처리할 수 있는 핸들러가 존재하지 않습니다. request = " + type };
	
	std::shared_ptr<IRequestHandler> handler = m_creators[type]->CreateHandler();
